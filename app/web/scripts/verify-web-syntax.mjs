...
function checkFile(filePath) {
  ...
  if (result.status !== 0) {
    ...
    console.error(`[web-syntax] Syntax check failed for ${filePath}`);
    if (stdout.trim()) {
...
if (files.length === 0) {
  console.error("[web-syntax] No source files found to check.");
  process.exit(1);
}
...
console.log(`[web-syntax] Syntax checks passed (${String(files.length)} files).`);
