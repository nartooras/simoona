...
try {
  ...
  const employeeListResponse = await request("/default/Employee/List");
  assert(
    employeeListResponse.statusCode === 200,
    `Expected /default/Employee/List to return 200, got ${String(employeeListResponse.statusCode)}.`
  );
  assert(
    employeeListResponse.body.includes('id="simoona-runtime-data"'),
    "Employee list response is missing runtime payload marker."
  );
  assert(
    employeeListResponse.body.includes("\"employeeList\""),
    "Employee list response is missing employeeList runtime payload."
  );
  assert(
    employeeListResponse.body.includes('src="/src/main.tsx"'),
    "Employee list response is missing runtime client script reference."
  );
  ...
  run(
    ...
    [
      ...
      "playwright",
      "test",
      "./scripts/employee-list-runtime.spec.js",
      "--workers=1",
      "--reporter=line",
      "--browser=chromium"
    ],
    {
      env: {
        ...process.env,
        EMPLOYEE_LIST_BASE_URL: webBaseUrl
      }
    }
  );

  console.log("[employee-list-runtime] Runtime behavior and visual evidence captured.");
} finally {
  shutdown();
