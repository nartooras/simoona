# QA Report: thread-j-api-general-settings-endpoint

## Scope
- Validated branch: `codex/thread-j-api-general-settings-endpoint`
- Reviewer decision gate required: `CONTINUE_TO_QA`
- Reviewer decision observed: `CONTINUE_TO_QA` (from `modern/docs/reviews/thread-j-api-general-settings-endpoint-review.md`)
...
- API smoke:
  ...
  - With local JWT env vars (`Auth__Jwt__Issuer`, `Auth__Jwt__Audience`, `Auth__Jwt__SigningKey`, `Auth__Jwt__Authority`) API started:
    ...
    - General settings endpoint probe (`/api/v1/user/general-settings` with only `X-Org-Id`): `401`
...
- New API endpoint work remains in `modern/**` and aligns with modernization boundary constraints.

## 5) Risks / Gaps
- API local startup depends on explicit JWT env configuration; missing values cause startup failure.
