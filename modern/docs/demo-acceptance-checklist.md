...
- Parity-acceptable now:
  - Shell and wall-route fidelity are consistent enough for stakeholder walkthroughs at desktop demo widths.
...
## Wall Navigation Prototype

- Dedicated routes: `/` (`Official wall`), `/walls` (`All walls`), and subscribed wall feeds (`/walls/<id>`), all classified `mock` / `mock-backed`.
- Demo expectations:
  - `All walls` directory shows official, subscribed, and unavailable contexts without blank states
  - subscribed wall routes deterministically switch between `Engineering`, `People`, and `Newcomers` contexts
...
| Write-scope expectations | No persistent writes are presented as available; deferred write-heavy flows are explicitly called out. | `met` | `/service-requests` and `/externals/integrations` remain intentionally unavailable; wall interactions stay local-only/non-persistent. |
